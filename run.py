...
class TimeRange():
    def __init__(self, start_time, end_time, min_interval, sum_interval, max_interval, duration, completed_in, success, failed):
...
def print_stats(previous_data, min_data, max_data, ipstats, gtpuTrafficgen, interfaces_map, ue_fg_msg_states):
    try:
        print()
...
def main(args: Arguments):
    ...
    ue_fg_msg_states = multiprocessing.Array('i', tuple([0] * (FGSM_MAX_TYPE - FGMM_MIN_TYPE + 1)), lock=True)
    exit_program = multiprocessing.Value('i', 0)

    processes = []
    ue_lists, num_ues = create_ues(ue_config['ue_profiles'], num_cpus - 1 if num_cpus > 1 else 1)
    for i in range(len(ue_lists)):
        ...
        config = {}
        gnb = GNB(exit_program, sctp_client, gtpu, server_config, ngap_to_ue, ue_to_ngap, upf_to_ue, ue_to_upf, args.verbose)
        ueSim = UESim(ue_fg_msg_states, exit_program, ue_lists[i], ngap_to_ue, ue_to_ngap, upf_to_ue, ue_to_upf, args.interval, args.statistics, args.verbose, ue_sim_time)
        # % num_cpus so that on single CPU, affirnity is set to CPU 0
        processes.append(Process(name=f"ueSIM-{i}", target=ueSim.run, args=((i+1) % num_cpus,)))
        processes.append(Process(name=f"gnb-{i}", target=gnb.run, args=((i+1) % num_cpus,)))
    ...
    while True:
        if args.statistics:
            print_stats(previous_data, min_data, max_data, ipstats, gtpuTrafficgen, interfaces_map, ue_fg_msg_states) 
    ...
    table = [["Duration",f" {ue_sim_time.end_time.value - ue_sim_time.start_time.value} seconds"],
    # ["Completed in",f" {latest_time - start_time} seconds"],
    ["N# of UEs",num_ues],["Successful procedures ",f"{ue_sim_time.success.value} UEs"],
    ["Failed procedures",f"{num_ues - ue_sim_time.success.value} UEs"],["Min interval",f"{ue_sim_time.min_interval.value} seconds"],
    ["Avg interval",f"{ue_sim_time.sum_interval.value/ue_sim_time.success.value if ue_sim_time.success.value > 0 else -1} seconds"], ["Max interval",f"{ue_sim_time.max_interval.value} seconds"]]
    print("\n\n")
